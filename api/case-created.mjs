...
async function getInitialResponseAdvice(c) {
  ...
  const system = [
    ...
    '- 過去の類似案件（同じ設備・同じ症状）は「内容」「社内メモ」「社内アドバイス」から探し、当時起きたこと・ハマった点を短く引用する。無ければ無いと書く。',
    '- 「社内アドバイス（人が確定した正しい対応）」があれば、最も信頼できる正解として最優先で反映する。'
  ].join('\n');
  ...
  const user = [
    ...
    '',
    `# 社内アドバイス（${c.theater || '不明'}／人が確定した正しい対応・最優先で反映）`,
