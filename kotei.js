...
/* ============================================================
   ...
   ・1日4分割（〜8時 / 8〜12時 / 12〜17時 / 17時〜）
   ・各作業4行（日中詳細／日中バー(青)／夜間詳細／夜間バー(赤)）
   ・ダブルクリックで文字入力（バーと文字は連動）／上下移動で日中⇔夜間
   ・Ctrl+クリックで複数選択／戻す・進む／縮尺変更／特記事項／印刷／Excel出力
   ・保存先は Supabase の koutei テーブル（現場ごと独立・全員共有）
   ============================================================ */
(function () {
  const BASE_QW = 16, SLOTS_PER_DAY = 4;
  let scale = 1;
  let QW = BASE_QW, DAY_W = QW * SLOTS_PER_DAY;
  ...
  function defaultState() {
    return {
      v: 2, title: "工事名　工程表", content: "", start: isoToday(), days: 21,
      author: "", notes: "",
  ...
  function applyScaleVars() {
    QW = Math.round(BASE_QW * scale); DAY_W = QW * SLOTS_PER_DAY;
    if (viewEl) { viewEl.style.setProperty('--kqw', QW + 'px'); viewEl.style.setProperty('--kday', DAY_W + 'px'); }
  ...
  function render() {
    ...
    for (let i = 0; i < total; i++) {
      const di = dayInfo(i);
      const day = el('div', 'kday'); day.style.width = DAY_W + 'px'; day.style.flex = '0 0 ' + DAY_W + 'px';
      ...
      // 区切り線（8時/12時/17時の縦線）＋ ラベル
      [1, 2, 3].forEach((q) => { const ln = el('span', 'kt-line'); ln.style.left = (q * QW) + 'px'; times.appendChild(ln); });
      [['8時', 1], ['12時', 2], ['17時', 3]].forEach(([t, q]) => {
        const s = el('span', 'kt'); s.textContent = t; s.style.left = (q * QW + 1) + 'px'; times.appendChild(s);
      });
    ...
    state.tasks.forEach((task) => {
      ...
      state.bars.filter((b) => b.taskId === task.id).forEach((b) => {
        (b.night ? barNight : barDay).appendChild(makeBar(b));
        if (b.label) (b.night ? detailNight : detailDay).appendChild(makeDetailText(b));
      });
  ...
  function onBarDown(e, b, div) {
    ...
    if (e.ctrlKey || e.metaKey) { // Ctrl/⌘+クリックで複数選択トグル
  ...
  function onDragMove(e) {
    ...
    if (drag.mode === 'move') {
      drag.group.forEach((g) => { g.bar.startSlot = clamp(g.origStart + dSlots, 0, total - g.bar.lenSlots); liveUpdate(g.bar); });
    } else if (drag.mode === 'resize-r' || drag.mode === 'create') {
      b.lenSlots = clamp(drag.origLen + dSlots, 1, total - b.startSlot); liveUpdate(b);
    } else if (drag.mode === 'resize-l') {
      const ns = clamp(drag.origStart + dSlots, 0, drag.origStart + drag.origLen - 1);
      b.lenSlots = drag.origStart + drag.origLen - ns; b.startSlot = ns; liveUpdate(b);
    }
  }
  function onDragUp(e) {
    ...
    if (drag) {
      ...
      if (drag.mode === 'move' && drag.group.length === 1) {
        ...
        if (lane && (lane.dataset.task !== b.taskId || (lane.dataset.night === '1') !== b.night)) {
          b.taskId = lane.dataset.task; b.night = lane.dataset.night === '1'; drag.reassigned = true;
        }
      }
      if (drag.mode === 'create' && !drag.moved) b.lenSlots = SLOTS_PER_DAY;
      const changed = drag.moved || drag.reassigned || drag.mode === 'create';
      if (changed) { recordUndo(drag.before); save(); render(); }
      else applySelection(); // ただのクリック→再描画しない（ダブルクリック成立のため）
    }
    drag = null;
  }
  ...
  function editBar(b, div) {
    const lane = div.parentElement;
    const detail = lane.previousElementSibling;
    if (!detail) return;
    const before = snap();
  ...
  function updateUndoButtons() {
    const u = $('#viewKotei #btnUndo'), rd = $('#viewKotei #btnRedo');
    if (u) u.disabled = !undoStack.length;
    if (rd) rd.disabled = !redoStack.length;
  }
  ...
  bindField('kContent', 'content');
  bindField('kAuthor', 'author');
  bindField('kNotes', 'notes');
  bindField('start', 'start', { render: true });
  bindField('days', 'days', { num: true, render: true });
  ...
  function fillPrintHead() {
    ...
    set('phStart', fmtJp(state.start)); set('phEnd', fmtJp(endDateIso())); set('phAuthor', state.author);
  }
  ...
  /* ============================================================
     保存・読込（Supabase koutei / local・複数現場）
     ============================================================ */
  ...
  function exportExcel() {
    ...
    const cellMap = {}; const merges = []; const rowMeta = [];
    ...
    put(r, 1, '工事名', S.meta); put(r, 2, state.title || '', S.metaVal); put(r, half, '着工日', S.meta); put(r, half + 1, fmtJp(state.start), S.metaVal); r++;
    put(r, 1, '工事内容', S.meta); put(r, 2, state.content || '', S.metaVal); put(r, half, '竣工日', S.meta); put(r, half + 1, fmtJp(endDateIso()), S.metaVal); r++;
    ...
    for (let i = 0; i < days; i++) {
      const di = dayInfo(i); const c0 = 2 + i * SLOTS_PER_DAY; const st = (di.isWeekend || di.isHoliday) ? S.headerWk : S.header;
      put(dateRow, c0, di.md + '(' + di.dowName + ')', st);
      for (let q = 1; q < SLOTS_PER_DAY; q++) put(dateRow, c0 + q, '', st);
    ...
    for (let i = 0; i < days; i++) {
      const c0 = 2 + i * SLOTS_PER_DAY; const wk = (dayInfo(i).isWeekend || dayInfo(i).isHoliday);
      put(timeRow, c0, '', wk ? S.headerWk : S.time); put(timeRow, c0 + 1, '8時', wk ? S.headerWk : S.time);
      put(timeRow, c0 + 2, '12時', wk ? S.headerWk : S.time); put(timeRow, c0 + 3, '17時', wk ? S.headerWk : S.time);
    }
    setH(timeRow, 18); r++;

    state.tasks.forEach((task) => {
      const rDD = r, rBD = r + 1, rDN = r + 2, rBN = r + 3;
      put(rDD, 1, task.name, S.name); merges.push('A' + rDD + ':A' + rBN);
      for (let i = 0; i < days; i++) {
        const wk = (dayInfo(i).isWeekend || dayInfo(i).isHoliday);
        for (let q = 0; q < SLOTS_PER_DAY; q++) { const c = 2 + i * SLOTS_PER_DAY + q;[rDD, rBD, rDN, rBN].forEach((rr) => put(rr, c, '', wk ? S.wk : S.grid)); }
      }
      state.bars.filter((b) => b.taskId === task.id).forEach((b) => {
        const barRow = b.night ? rBN : rBD, detRow = b.night ? rDN : rDD, cStart = 2 + b.startSlot;
        for (let kk = 0; kk < b.lenSlots; kk++) put(barRow, cStart + kk, '', b.night ? S.night : S.day);
        if (b.label) put(detRow, cStart, b.label, S.detail);
      });
      setH(rDD, 16); setH(rBD, 16); setH(rDN, 16); setH(rBN, 16);
      r += 4;
    });
    ...
    for (let rr = 1; rr <= lastRow; rr++) {
      ...
      list.forEach((cell) => {
        const ref = colLetter(cell.c) + rr; const sAttr = cell.s ? ' s="' + cell.s + '"' : '';
  ...
  function buildStyles() {
    ...
    const cellXfs = '<cellXfs count="14">' +
