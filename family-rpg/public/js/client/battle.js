...
export class BattleScene {
  ...
  renderStatus(full = false) {
    ...
    if (full || this.statusBoxes.size !== allies.length) {
      ...
      for (const a of allies) {
        ...
        const nm = el('div', { class: 'nm' }, el('span', { text: a.name }), el('span', { class: 'job', text: `${JOBS[a.job]?.name?.slice(0, 2) || ''}${a.lv}` }));
    ...
    for (const a of allies) {
      ...
      s.sts.textContent = !a.alive ? 'しに' : a.defending ? `ぼうぎょ ${st}` : (st || (a.auto && a.controller ? 'オート' : a.kind === 'support' ? 'サポート' : a.kind === 'guest' ? 'ゲスト' : ''));
    }
  }
  ...
  openCommand() {
    ...
    const skills = learned.filter((id) => ABILITIES[id] && (ABILITIES[id].kind === 'skill' || (ABILITIES[id].kind === 'combo' && !ABILITIES[id].spellLike)));
    const items = [
  ...
  abilityMenu(ids) {
    const a = this.myActor;
    const pc = { job: a.job, jobs: this.game.me.jobs };
  ...
  mahoukenMenu() {
    const a = this.myActor;
    const pc = { job: a.job, jobs: this.game.me.jobs, equip: this.game.me.equip };
    const opts = mahoukenOptions({ ...this.game.me, job: a.job }, a.abilities);
  ...
  toggleAuto() {
    const a = this.myActor;
    if (!a) return;
    this.game.net.send({ t: 'battle', actor: a.id, auto: !a.auto });
  }
    ...
    for (const ev of evs) {
      switch (ev.t) {
        ...
        case 'ready': {
          ...
          if (this.mine.includes(ev.id)) this.openCommand();
          this.renderStatus();
          break;
        }
        case 'queued': {
  ...
  present(ev) {
    ...
    if (actor && this.mine.includes(actor.id)) this.renderCmdIdle();
    // エフェクト
    ...
    if (anim && enemyPts.length) this.fx.play(anim, enemyPts, fx.element);
    ...
    if (fx.team) this.banner(`合体！ ${fx.team}！`);
    ...
    else if (fx.type === 'ability' && fx.side === 'ally' && !ANIM_SFX[anim]) g.audio.sfx('spell');
    ...
    for (const r of ev.results || []) {
      ...
      if (r.dmg > 0) {
        if (t.side === 'enemy') {
          t.flash = 320;
          if (!fx.anim || fx.type === 'attack') g.audio.sfx(r.crit ? 'crit' : 'hit');
        } else {
          hurtAlly = true;
          this.hitStatus(t.id);
        ...
        if (t.side === 'ally') this.glowStatus(t.id, '#7dffb0');
        else this.fx.play('heal1', [this.center(t)].filter(Boolean));
      } else if (r.miss || r.dmg === 0) {
        this.floatNum(t, 'ミス', 'miss');
        g.audio.sfx('miss');
      }
    }
    if (hurtAlly && actor?.side === 'enemy' && (actor.boss || (ev.results || []).some((r) => r.crit))) this.shake(350);
    ...
    const mine = this.myActor;
    if (mine && this.menu && (!mine.alive || (mine.status || []).some((x) => x === 'sleep' || x === 'paralyze') || !mine.ready)) this.renderCmdIdle();
    this.renderStatus();
    this.updateAutoBtn();
  }
